...
class BBBacktester():
    ...
    def test_strategy(self):
        ...
        data["distance"] = data.price - data.SMA # helper colunm
        # 1. price cross down the lower bound ? signal to BUY (1) 
        data["position"] = np.where(data.price < data.Lower, 1, np.nan)
        # 2. price cross up the upper bound ? signal to SELL (-1) 
        data["position"] = np.where(data.price > data.Upper, -1, data["position"])
        # 3. check if we can close the positions (go neutral):
        # - if the distance change the sign, its mean that we can go neutral (from long/short) 
        # - else, we can leave the position as is. 
        # e.g:
        # -1 * -2 = 1: mean that we need still stay on the short position.
        # 4 * 3 = 12: mean that we still need stay on the long position
        # -4 * 3 = -12: the distance change its sign, go neutral, finish the position. 
        data["position"] = np.where(data.distance * data.distance.shift(1) < 0, 0, data["position"])
        # 4. if 1-3 not satisfied, fill the nan with 0 - stay nuetral there.
